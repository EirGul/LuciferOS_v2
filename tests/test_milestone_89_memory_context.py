...
def build_search_results():
    ...
    return retrieval.search(MemoryQuery(text='LuciferOS PowerShell', limit=5))
...
def test_memory_context_builder_trims_long_memory_content():
    ...
    results = MemoryRetrievalService(store).search(MemoryQuery(limit=1))
    context = MemoryContextBuilder(max_chars_per_item=50).build(results)
